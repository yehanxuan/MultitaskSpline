...
load("Dataset/spectra/spcData.gzip")
load("Dataset/spectra/spcTable.RData")
...
if (method == "PSpline"){
  mOrder = 4
  Mont = dim(X)[2]
  lambdaSeq = exp(seq(-45, -17, length.out = 7)) 
  rankSeq = c(1:3)
  Select = Spline_Selection(X_train, Y_train, mOrder, nKnots, lambdaSeq, rankSeq, Kfold)
  fit = ReduceRankSpline(X_train, Y_train, mOrder, nKnots, Select$opt_rank,
                         Select$opt_lambda)
  ...
  fit = FPCA_Selection_Comp(X_train, Y_train, pSeq, Select_Method = "CV", Kfold)
 # fit = FPCA_Estimate_Multitask(X_test, Y_test, Select$pSelect)
 # fit = FPCA_Graph(X_train, Y_train, pSeq, Kfold, Select_Method = "CV")
  ...
  fit_Tikhonov = Tikhonov_Selection_Comp(X_train, Y_train, RhoSeq, Kfold, Select_Method = "CV")
